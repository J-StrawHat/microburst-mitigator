...
def plot_lost(opt_str, data_0, data_1, filename, bg, ed, step):
    x_arr = data_0['Aggregated Network Load (%)']
    y_arr_0 = data_0[opt_str]
    y_arr_1 = data_1[opt_str]
    y_ticks = []
    t = bg
    for i in range(0, int((ed - bg) / step)):
        y_ticks.append(t)
        t += step
...
bg_filename = 'bg25_bg_bgn20_burstn5'
burst_filename = 'bg25_burst_bgn20_burstn5'
...
plot_lost(opt_strs[3], bg_df0, bg_df1, bg_filename, 0, 0.2, 0.05)
plot_lost(opt_strs[3], burst_df0, bg_df1, burst_filename, 0, 0.2, 0.05)
