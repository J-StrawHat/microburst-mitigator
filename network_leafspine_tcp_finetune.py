...
def run_measurement(net, deflect_mode = 0, threshold = 25, bg_size = 20, burst_size = 5):
    ...
    for bg_load in bg_load_list:
        print('\033[96m' + "===  round begin (bg:%f Mbps, %d MBytes) (burst:%f Mbps, %d MBytes) ===" % (0.01 * bg_load * leaf_bw, bg_size, 0.01 * (95 - bg_load) * leaf_bw, burst_size) + '\033[0m')
        bg_test_res, burst_test_res = run_iperf_loop(net, 
                                                    bg_bw = 0.01 * bg_load * leaf_bw, 
                                                    burst_bw = 0.01 * (95 - bg_load) * leaf_bw,
                                                    bg_size = bg_size,
    ...
    fa_dir = fa_dir_pre + str(deflect_mode) + "_t" + str(t)
    if not os.path.exists(fa_dir):
        os.makedirs(fa_dir)
    bg_rows = zip(bg_load_list, bg_fcts_list, bg_retrans_avg_list, bg_retrans_p95_list)
    bg_log_filename = fa_dir + '/tcp_result_bg_bgn%d_burstn%d.csv' % (bg_size, burst_size)
    burst_rows = zip(bg_load_list, burst_fcts_list, burst_retrans_avg_list, burst_retrans_p95_list)
    burst_log_filename = fa_dir + '/tcp_result_burst_bgn%d_burstn%d.csv' % (bg_size, burst_size)

    with open(bg_log_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Background Load (%)', 'Mean FCT (s)', 'Mean number of packet retransmission', 'P95 number of packet retransmission'])
...
for t in range(4, 64, 4):
    ...
    output = template.render(deflect_mode=2, threshold = t)
    #print(output)
    ...
    run_measurement(net, deflect_mode = 2, threshold = t, bg_size=background_flow_size, burst_size=burst_flow_size)
